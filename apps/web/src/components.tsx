...
export const LoginModal = () => {
  ...
  return <Modal open={!accessToken} login title="TLITODOS에 오신 걸 환영해요">
    ...
    <GoogleButton type="button" onClick={login} disabled={loading}><GoogleMark>G</GoogleMark>{loading ? "로그인 중..." : "Google로 로그인하기"}</GoogleButton>
  </Modal>;
};
const LoginCopy = styled.p`margin:-14px 0 28px;color:${theme.colors.muted};`;
const GoogleButton = styled.button`width:100%;display:flex;align-items:center;justify-content:center;gap:12px;border:1px solid ${theme.colors.line};border-radius:14px;background:white;padding:15px;font-weight:700;&:hover{background:#f8fafb;}`;
const GoogleMark = styled.span`font-family:Arial,sans-serif;font-size:22px;color:#4285f4;font-weight:800;`;

export const WorkspaceHeader = ({ activeGroupId, onCreate, onJoin }: { activeGroupId?: number; onCreate: () => void; onJoin: () => void }) => {
  const navigate = useNavigate();
  const { data: me } = useMe();
  const { data: groups = [] } = useGroups();
  return <HeaderRow>
    <ViewChip active={activeGroupId === undefined} avatar={me?.profileImageUrl} onClick={() => navigate("/")}>{me?.name || "나의 TODO"}</ViewChip>
    {groups.map((group) => <ViewChip key={group.groupId} active={activeGroupId === group.groupId} onClick={() => navigate(`/groups/${group.groupId}`)}>{group.name}</ViewChip>)}
    <Button type="button" onClick={onCreate}>＋ 그룹 생성</Button>
    <Button type="button" variant="ghost" onClick={onJoin}>초대코드로 참여하기</Button>
  </HeaderRow>;
};
...
export const CalendarPanel = ({ month, selectedDate, todos, categories, onMonthChange, onDateChange }: { month: Date; selectedDate: string; todos: Todo[]; categories: Category[]; onMonthChange: (date:Date)=>void; onDateChange:(date:string)=>void }) => {
  const sorted = sortCategories(categories); const days = getCalendarDays(month); const today = formatLocalDate(new Date());
  return <CalendarWrap>
    <MonthHeader><MonthInput type="month" value={`${month.getFullYear()}-${String(month.getMonth()+1).padStart(2,"0")}`} aria-label="월 빠른 이동" onChange={(e)=>{const [y,m]=e.target.value.split("-").map(Number);if(y&&m)onMonthChange(new Date(y,m-1,1));}}/><MonthButtons><IconButton onClick={()=>onMonthChange(addMonths(month,-1))}>‹</IconButton><IconButton onClick={()=>onMonthChange(addMonths(month,1))}>›</IconButton></MonthButtons></MonthHeader>
    <WeekRow>{["월","화","수","목","금","토","일"].map((day,index)=><span key={day} data-weekend={index>4}>{day}</span>)}</WeekRow>
    <DaysGrid>{days.map((date,index)=>{
      if(!date)return <span key={`empty-${index}`}/>;
      const value=formatLocalDate(date); const dayTodos=todos.filter((todo)=>sameLocalDate(todo.dueDate,value));
      const tones:CategoryTone[]=[]; const completed:CategoryTone[]=[];
      sorted.forEach((category,catIndex)=>{const list=dayTodos.filter((todo)=>todo.categoryId===category.categoryId);if(list.length){const tone=categoryToneAt(catIndex);tones.push(tone);if(list.every((todo)=>todo.isCompleted))completed.push(tone);}});
      return <DayStash key={value} date={date.getDate()} tones={tones} completed={completed} selected={value===selectedDate} today={value===today} onClick={()=>onDateChange(value)}/>;
    })}</DaysGrid>
  </CalendarWrap>;
};
const CalendarWrap = styled.section`width:min(450px,100%);`;
const MonthHeader = styled.div`display:flex;align-items:center;justify-content:space-between;padding-bottom:14px;border-bottom:1px solid #d7dee5;`;
const MonthInput = styled.input`border:0;background:transparent;font-size:20px;font-weight:800;color:${theme.colors.ink};&::-webkit-calendar-picker-indicator{opacity:.45;cursor:pointer;}`;
const MonthButtons = styled.div`display:flex;gap:5px;`;
const WeekRow = styled.div`display:grid;grid-template-columns:repeat(7,1fr);text-align:center;margin:15px 0 7px;font-size:13px;font-weight:700;span:nth-of-type(6){color:${theme.colors.blue};}span:nth-of-type(7){color:${theme.colors.red};}`;
...
const CalendarChooser = ({ value, onChange }: { value:string; onChange:(next:string)=>void }) => {
  ...
  return <MiniCalendar><MonthHeader><MonthInput type="month" value={`${month.getFullYear()}-${String(month.getMonth()+1).padStart(2,"0")}`} onChange={(e)=>{const [y,m]=e.target.value.split("-").map(Number);if(y&&m)setMonth(new Date(y,m-1,1));}}/><MonthButtons><IconButton onClick={()=>setMonth(addMonths(month,-1))}>‹</IconButton><IconButton onClick={()=>setMonth(addMonths(month,1))}>›</IconButton></MonthButtons></MonthHeader><MiniDays>{["월","화","수","목","금","토","일"].map((day)=><b key={day}>{day}</b>)}{days.map((date,index)=>date?<button type="button" data-selected={formatLocalDate(date)===value} onClick={()=>onChange(formatLocalDate(date))} key={formatLocalDate(date)}>{date.getDate()}</button>:<span key={`e-${index}`}/>)}</MiniDays></MiniCalendar>;
};
const MiniCalendar = styled.div`margin-top:16px;border:1px solid ${theme.colors.line};border-radius:18px;padding:16px;`;
const MiniDays = styled.div`display:grid;grid-template-columns:repeat(7,1fr);gap:4px;margin-top:10px;text-align:center;b{font-size:11px;color:${theme.colors.muted};}button{height:34px;border:0;border-radius:50%;background:transparent;}button[data-selected='true']{background:#dff6ad;font-weight:800;}`;

export const DeadlineModal = ({ open, value, onChange, onClose }: {open:boolean;value:DeadlineValue;onChange:(next:DeadlineValue)=>void;onClose:()=>void}) => {
  const [panel,setPanel]=useState<"date"|"time"|null>(null);
  return <Modal open={open} nested title="마감기한 설정하기">
    <ToggleRow><Button onClick={()=>setPanel(panel==="date"?null:"date")}>📅 {value.date.replaceAll("-","/")}</Button><Button onClick={()=>setPanel(panel==="time"?null:"time")}>⏰ {value.time||"시간 설정"}</Button></ToggleRow>
    {panel==="date"?<CalendarChooser value={value.date} onChange={(date)=>onChange({...value,date})}/>:null}
    {panel==="time"?<TimeInput type="time" value={value.time} onChange={(e)=>onChange({...value,time:e.target.value})}/>:null}
    <ButtonStack><Button variant="primary" onClick={onClose}>설정 완료</Button></ButtonStack>
  </Modal>;
};
const ToggleRow = styled.div`display:flex;gap:10px;flex-wrap:wrap;`;
const TimeInput = styled.input`display:block;width:100%;margin-top:18px;border:1px solid ${theme.colors.line};border-radius:12px;padding:14px;`;

export interface RoutineValue { start:string; end:string; time:string; repeat:RoutineRepeat }
export const RoutineModal = ({ open, initialDate, onRegister, onClose }: {open:boolean;initialDate:string;onRegister:(value:RoutineValue)=>Promise<void>;onClose:()=>void}) => {
  ...
  return <Modal open={open} nested title="루틴으로 등록하기">
    ...
    {panel==="time"?<TimeInput type="time" value={value.time} onChange={(e)=>setValue({...value,time:e.target.value})}/>:null}
    {panel==="repeat"?<ChoiceRow>{([['DAILY','매일'],['WEEKDAYS','평일'],['WEEKLY','매주']] as const).map(([key,label])=><Option key={key} name="repeat" value={key} checked={value.repeat===key} onChange={()=>setValue({...value,repeat:key})} label={label}/>)}</ChoiceRow>:null}
    <ButtonStack><Button variant="primary" disabled={busy||value.end<value.start} onClick={async()=>{setBusy(true);try{await onRegister(value);}finally{setBusy(false);}}}>{busy?"등록 중...":"루틴으로 등록하기"}</Button><Button onClick={onClose}>취소</Button></ButtonStack>
  </Modal>;
};
...
export const TodoEditorModal = ({ open, selectedDate, initialCategory, todo, categories, todos, onClose, onSaved }: {open:boolean;selectedDate:string;initialCategory:Category|null;todo:Todo|null;categories:Category[];todos:Todo[];onClose:()=>void;onSaved?:()=>void}) => {
  ...
  return <>
    <Modal open={open} title={todo?"할 일 수정하기":"할 일 추가하기"} onClose={onClose}>
      <FormGrid><div><Field>할 일 {"*"}<input value={title} maxLength={15} onChange={(e)=>setTitle(e.target.value)} placeholder="할 일을 추가하세요..."/><small>{title.length}/15</small></Field><Field>할 일에 대한 세부사항<input value={detail} maxLength={20} onChange={(e)=>setDetail(e.target.value)} placeholder="세부사항을 작성하세요..."/><small>{detail.length}/20</small></Field>
      <Question>Q. 이 일을 하기 전 선행해야 할 일이 있나요?</Question><DependencyList>{candidates.length?candidates.map((candidate)=>{const index=sortCategories(categories).findIndex((category)=>category.categoryId===candidate.categoryId);const preset=CATEGORY_PRESETS[index]??CATEGORY_PRESETS[0];return <label key={candidate.todoId}><input type="radio" name="dependency" checked={dependency===candidate.todoId} onChange={()=>setDependency(candidate.todoId)}/><i style={{borderColor:preset.color,background:dependency===candidate.todoId?preset.color:'transparent'}}/>{candidate.title}</label>;}):<small>선택할 수 있는 선행 할 일이 없습니다.</small>}</DependencyList></div>
      <div><Question>카테고리 설정하기</Question><ChoiceRow>{sortCategories(categories).map((category)=><Selection key={category.categoryId} name="category" value={category.categoryId} checked={categoryId===category.categoryId} onChange={()=>setCategoryId(category.categoryId)} label={category.name}/>)}</ChoiceRow>
      <Question>그룹 안 공개 범위 설정하기 {"*"}</Question><ChoiceRow><Selection name="visibility" checked={visibility==="GROUP"} onChange={()=>setVisibility("GROUP")} label="전체 공개"/>{/* 일부 공개는 MVP 이후 활성화 */}<Selection name="visibility" checked={visibility==="PRIVATE"} onChange={()=>setVisibility("PRIVATE")} label="비밀"/></ChoiceRow>
      {!selectedCategory||!isHobbyCategory(selectedCategory)?<><Question>중요도(우선순위) 설정하기 {"*"}</Question><ChoiceRow>{([['NONE','선택하지 않음'],['HIGH','높음'],['LOW','낮음']] as const).map(([value,label])=><Selection key={value} name="importance" checked={importance===value} onChange={()=>setImportance(value)} label={label}/>)}</ChoiceRow></>:null}
...
const DependencyList = styled.div`display:grid;gap:12px;label{display:flex;align-items:center;gap:10px;cursor:pointer;}input{position:absolute;opacity:0;}i{width:22px;height:22px;border-radius:50%;border:2px solid;display:block;}small{color:${theme.colors.muted};}`;
...
export const CategorySection = ({ category, index, todos, own, onAdd, onManage, onToggle, onEdit }: {category:Category;index:number;todos:Todo[];own:boolean;onAdd:(category:Category)=>void;onManage:(category:Category)=>void;onToggle:(todo:Todo)=>void;onEdit:(todo:Todo)=>void}) => {
  ...
  return <CategoryColumn><CategoryPill name={category.name} tone={tone} own={own} onAdd={own?()=>onAdd(category):undefined} onManage={own&&index>0&&index<3?()=>onManage(category):undefined}/><TodoList><TodoRows todos={todos} tone={tone} own={own} onToggle={onToggle} onEdit={onEdit}/></TodoList></CategoryColumn>;
};
// Kept separate so each row receives stable action closures.
const TodoRows=({todos,tone,own,onToggle,onEdit}:{todos:Todo[];tone:CategoryTone;own:boolean;onToggle:(todo:Todo)=>void;onEdit:(todo:Todo)=>void})=><>{todos.map((todo)=><SharedTodoRow key={todo.todoId} todo={todo} tone={tone} own={own} onToggle={()=>onToggle(todo)} onEdit={()=>onEdit(todo)}/>)}</>;
const CategoryColumn = styled.section`min-width:0;`;
const TodoList = styled.div`display:grid;gap:14px;margin-top:20px;`;
