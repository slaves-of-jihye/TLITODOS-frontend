...
export const getCalendarDays = (viewDate: Date) => {
  ...
  return [
    ...
    ...Array.from({ length: last.getDate() }, (_, index) => new Date(viewDate.getFullYear(), viewDate.getMonth(), index + 1)),
  ];
};
...
export const isTodoCategory = (category: Pick<Category, "name">) => ["해야할일", "할일", "과제"].includes(normalizeName(category.name));
export const isHobbyCategory = (category: Pick<Category, "name">) => normalizeName(category.name) === "취미";
export const isLockedCategory = (category: Pick<Category, "name">) => isTodoCategory(category) || isHobbyCategory(category);
...
export const sortTodos = (todos: Todo[]) => [...todos].sort((a, b) => importanceRank[a.importance] - importanceRank[b.importance] || a.todoId - b.todoId);

export const todosForDate = (todos: Todo[], selectedDate: string) => todos.filter((todo) => sameLocalDate(todo.dueDate, selectedDate));

export const unresolvedDependencies = (todo: Todo, allTodos: Todo[]) => todo.dependencies
...
export const buildRoutineDates = (start: string, end: string, repeat: RoutineRepeat) => {
  ...
  while (cursor <= last) {
    ...
    if (repeat === "DAILY" || (repeat === "WEEKLY" && day === parseLocalDate(start).getDay()) || (repeat === "WEEKDAYS" && day > 0 && day < 6)) {
