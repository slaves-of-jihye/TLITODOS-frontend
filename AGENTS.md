...
- `할 일 삭제하기` opens a confirm dropdown under the button, never `window.confirm` — a routine occurrence has to choose between `이 할 일만 삭제` and `루틴 전체 삭제`, which a browser confirm cannot offer. Clicking anywhere outside the dropdown or pressing Esc is a cancel, and the trigger sits inside the region that counts as "inside" so pressing it again just closes the dropdown. `루틴 전체 삭제하기` keeps its own dropdown and opens it upward because it sits near the bottom of the sheet.
