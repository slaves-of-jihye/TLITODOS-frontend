...
- The category label sits on a neutral `#eef1f6` pill (`gray/200`); there is no per-category background any more. A pill you can rename darkens to `#dde2ec` (`gray/300`) on hover, which is the design's category hover variant, and carries a pointer cursor. Only that pill: a label with no action, like the ones on the profile screen, must not change under the pointer or it reads as clickable. The rule sits behind `@media (hover: hover) and (pointer: fine)` so a tap does not leave the colour stuck.
